...
router.post('/createData', async (req, res) => {
    try {
        ...
        const findMatchingOption = (data, name) => {
            for (const facility of data.facilities) {
                const option = facility.options.find(opt => opt.name === name);
                if (option) {
        ...
        const optionMatch = findMatchingOption(response, body.optionName);
        ...
        return res.status(200).send({ status: true, message: "Data processed successfully." });
